...
Simply put, speed. When the task that we are trying to solve is [NP-Complete](https://en.wikipedia.org/wiki/NP-completeness), we can choose one of:
...
words = [next(w) for i, (_, w) in enumerate(words)]
